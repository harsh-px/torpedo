...
func (k *k8sSchedOps) ValidateVolumeSetup(vol *volume.Volume) error {
	...
	for _, p := range pods {
		...
		for containerName, path := range containerPaths {
			// TODO detect which container in pod is using the PVC
			...
			logrus.Infof("[debug] looking for %s in pod: [%s]%s", pxMountCheckRegex.String(), p.Namespace, p.Name)
			logrus.Infof("[debug] mounts in pod: [%s]%s are: %s", p.Namespace, p.Name, output)
			mounts := strings.Split(output, "\n")
			pxMountFound := false
			for _, line := range mounts {
				pxMounts := pxMountCheckRegex.FindStringSubmatch(line)
				if len(pxMounts) > 0 {
					logrus.Infof("[debug] pod: [%s] %s have PX mount: %v", p.Namespace, p.Name, pxMounts)
					pxMountFound = true
					break
				}
			}
			...
			if !pxMountFound {
				return fmt.Errorf("pod: [%s] %s does not have PX mount", p.Namespace, p.Name)
			}
		}
	}
